...
public class TfIdf {
    ...
    private static Pipeline buildPipeline( String bookDirectoryFullPath , String stopwordsTxtFile ) {
       Path bookDirectory = getClasspathDirectory( bookDirectoryFullPath );
        ...
        Set<String> stopwords = docLines(stopwordsTxtFile).collect(toSet());
    ...
    public static void main(String[] args) {
    	if (args.length < 2) {
            System.out.println("Sample Usage:  java TfIdf /tmp/books /tmp/stopwords.txt");
         } else {
            System.out.println("Yo " + args[0] + ", so you like " + args[1] + "?");
            
         }
    	try {
            new TfIdf().go( args[0] , args[1]);
        } catch (Throwable t) {
            Jet.shutdownAll();
            throw t;
        }
    ...
    private static Stream<String> docLines(String name) {
        try {
        	...
        	System.out.println("Stream: name is " + name);
